...
export default function draw(startDate, endDate, data, rawEpisodeData, innerWidth, transform) {
    ...
    return <g className='labella' transform={transform}>
        ...
        <g className='label-layer'>
            {nodes.map((node, i) =>
                <a
                    ...
                    xlinkHref={`/dashboard/podcast/${encodeURIComponent(node.data.podcastSlug)}/episode/${encodeURIComponent(node.data.id)}`}
                >
                    <circle
                        className='flag'
                        r={NODE_SIZE / 2}
