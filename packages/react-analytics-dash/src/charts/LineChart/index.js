...
import BaseD3Chart from '../BaseD3Chart';


export default class LineChart extends BaseD3Chart {
    constructor(props) {
        super(data);
    ...
    drawHoverTargets(lines) {
        const labelData = this.data.labels.map((label, i) => ({label, i}));
        const me = this;
    ...
    bindHoverTargetTooltip(elem, d) {
        const getBounding = () => this.node.getBoundingClientRect();
        const text = this.data.datasets.map(ds => `${ds.label}: ${ds.data[d.i]}`).join('<br>');
        this.bindTooltip(
            elem,
            `<b>${d.label}</b><br>${text}`,
