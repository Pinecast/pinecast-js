...
export default class BankForm extends Component {
  ...
  render() {
    const {props: {hasExisting}, state: {bankCountry, bankCurrency, collapsed, entityCountry, error, saving}} = this;
    ...
    if (collapsed) {
      return (
        ...
        >
          <button
            className="bank-form-update-account-btn"
    ...
    const availableCurrencies = currencies.countriesToCurrencies[entityCountry].filter(
      x => currencies.countriesToCurrencies[bankCountry].indexOf(x) !== -1,
    );
