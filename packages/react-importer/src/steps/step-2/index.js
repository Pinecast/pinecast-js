  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <p>
          {gettext(
            "This is the information that we have fetched from your podcast's RSS feed. Read it over and make sure everything is correct before continuing.",
          )}
        </p>
        ...
        <label>
          ...
          <input defaultValue={name} maxLength={256} ref="name" required type="text" />
        </label>
        <SlugField
          defaultValue={name
            .replace(/\s+/g, '-')
        ...
        <label>
          ...
          <input defaultValue={subtitle} ref="subtitle" type="text" />
        </label>
        <label>
          <span>{gettext('Description')}</span>
          <textarea defaultValue={description} ref="description" required />
        </label>
        ...
        <label>
          ...
          <input defaultValue={homepage} ref="homepage" type="url" />
        </label>

        <label>
          <span>{gettext('Language')}</span>
          <div className="select">
            <select defaultValue={language in LOCALES ? language : 'en-US'} ref="language">
        ...
        <label>
          ...
          <input defaultValue={copyright} ref="copyright" type="text" />
        </label>
        <label>
          <span>{gettext('Author')}</span>
          <input defaultValue={author} ref="author" type="text" />
        </label>
        ...
        <div className="panel">
          <p>
            {gettext(
              'These are the categories that are currently selected on your podcast. You can change your podcast categories after it has been imported.',
            )}
          </p>
        ...
        {Boolean(ignoredItems) && (
          <p style={{color: '#b00'}}>
            ...
            {gettext(
              ' Episodes like this do not have an <enclosure /> tag, which means there is no audio to download.',
            )}
          </p>
        )}
