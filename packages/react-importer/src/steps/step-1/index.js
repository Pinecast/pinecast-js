        ...
        xhr(
          ...
          (err, res, body) => {
            if (err || res.statusCode !== 200) {
              this.setState({
                error: gettext(
                  'We were not able to download your RSS feed. Please check that the feed URL is correct and try again later.',
                ),
                loading: false,
              });
              return;
    ...
    xhr(
      ...
      (err, res, body) => {
        if (err || res.statusCode !== 200) {
          this.setState({
            error: gettext(
              'There was a problem while processing the contents of your feed. Please contact Pinecast support.',
            ),
            loading: false,
          });
          return;
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <p>
          {gettext(
            'To begin, enter the RSS feed URL or Apple Podcasts directory URL of the podcast that you would like to import to Pinecast.',
          )}
        </p>

        <label>
          <span>{gettext('Podcast URL')}</span>
          <input placeholder="http://wtfpod.libsyn.com/rss" ref="url" required type="url" />
        </label>
