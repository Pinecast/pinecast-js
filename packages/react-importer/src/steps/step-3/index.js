...
export default class Step3 extends Component {
    ...
    xhr(
      {
        method: 'get',
        url:
          `/dashboard/services/import_progress/${encodeURIComponent(this.props.feed.slug)}` +
  ...
  getElementTaskType(elem) {
    ...
    return `${elem.entity.name} - ${elem.item_type === 'image'
            ...
            {Object.keys(elements).map(elemKey => {
              ...
              return (
                <tr key={elemKey}>
                  ...
                  <td>{this.getElementTaskStatus(elem)}</td>
                </tr>
              );
            })}
          </tbody>
